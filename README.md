...
Para el webhook de PayPal Live, configurá `PAYPAL_WEBHOOK_ID` en Vercel con el ID que PayPal entrega al crear el webhook. Listener:
`https://aprendiacurso.vercel.app/api/webhooks/paypal`

Eventos recomendados para Orders API: `PAYMENT.CAPTURE.COMPLETED`, `PAYMENT.CAPTURE.DENIED`, `PAYMENT.CAPTURE.PENDING`, `PAYMENT.CAPTURE.REFUNDED` y `CHECKOUT.PAYMENT-APPROVAL.REVERSED`.
